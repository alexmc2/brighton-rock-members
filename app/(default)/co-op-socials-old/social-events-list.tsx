...
export default function SocialEventsList({ events: initialEvents }: SocialEventsListProps) {
  const [events, setEvents] = useState<SocialEventWithDetails[]>(initialEvents || []);
  const [currentPage, setCurrentPage] = useState(1);
  const [categoryFilter, setCategoryFilter] = useState<'all' | SocialEventCategory>('all');
  const [statusFilter, setStatusFilter] = useState<'all' | SocialEventStatus>('all');
  ...
  const categories: Array<'all' | SocialEventCategory> = [
    'all',
    ...(Array.from(new Set(events.map((e) => e.category))) as SocialEventCategory[]),
  ];

  const statuses: Array<'all' | SocialEventStatus> = [
    'all',
    ...(Array.from(new Set(events.map((e) => e.status))) as SocialEventStatus[]),
  ];
